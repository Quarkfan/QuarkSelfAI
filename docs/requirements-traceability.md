...
| --- | --- | --- | --- |
| 代码可在任意受信终端 clone，并通过账号登录和数据备份恢复同一助理能力 | recovery-readiness, deployment-packaging, durable-store | 项目章程、ADR 0090、机器 recovery manifest、数据/身份清单、`audit:recovery`、age-only 打包、checksum/SQLite 校验与 revision 一致的 fresh-clone restore-safe 准备；代码远端已校验 | implemented/gated：SQLite 本地核心通过无秘密测试；仍需 owner 提供异机目标、age 身份并允许安装工具，再完成真实加密上传回读、PostgreSQL、账号只读检查和干净终端演练 |
