...
| 必要追问标注 AI 分身；正式回复必须先由本人确认 | context-and-external-guard, approval-cards | 策略/审批测试；现网 10 个卡片回调无重复、3 个待确认动作跨重启保留；追问回复读取网络故障局部降级、保留待处理项并恢复清标，不再终止 compatibility host；故障日志仅保留受限 category/subtype，回归拒绝 URL、会话标识、源地址和证书域名 | complete |
...
| 超期监控、完成任务定期清理、自动化跟进清单每工作日评估 | dida-monitors | 契约测试；三类 monitor 均有现网运行时间且当前健康；超期扫描、completed-cleanup 与工作日跟进超时均保留 Claude/Codex 主执行与兜底阶段的脱敏结果；跟进检查前两次失败静默、连续三次通知、仅已通知故障发送恢复 | complete：历史总超时已可在下一次失败时直接定位 provider 阶段，不读取或写入任务正文 |
| 跟进清单由助手跟踪和修改；联系他人前征求批准 | dida-monitors, approval-cards | 联系人解析、批准卡片、回复回写原任务测试；全部 workflow handoff CLI 通过统一策略从当前配置恢复精确 resource scope 与 owner 授权 revision；公开回执仅含固定长度 state/resource 哈希和计数，合成 CLI 回归覆盖 6 个入口 | gated：外联动作必须逐次批准；handoff 审计可在不写任务或启动 native workflow 的情况下验证恢复输入 |
...
| 会前主动汇集日历、责任、任务和资料，减少临时找上下文 | meeting-briefing-planner, capability-evolution | ADR 0170；闭合元数据输入、15 分钟至 24 小时责任门禁、opaque revision 幂等、来源数量上限、原始字段与 activation-shaped 输入失败关闭；纯计数退出门禁固定 8 个会议/10 个工作日、80% 覆盖、70% 有效、30% 低价值上限及任一安全违规失败；`pre-meeting-briefing-pilot-01` revision 3 以 SHA-256 指纹绑定请求、实现基线和单一 inactive 模块，审计拒绝 scope/phrase/notification revision/runtime/ownership 漂移 | candidate：规划、判定和授权审计均不读取真实来源/正文、不写简报、不通知；未获批准的 revision 1/2 已被内部一致的 revision 3 取代，静默只读影子试运行仍待 owner 精确批准，owner 可见交付须后续另批 |
