...
| --- | --- | --- | --- |
| 代码可在任意受信终端 clone，并通过账号登录和数据备份恢复同一助理能力 | recovery-readiness, account-bootstrap-readiness, deployment-packaging, durable-store | 项目章程、ADR 0090、机器 recovery/account manifest、数据/身份清单、真实 age-only 打包、iCloud provider 不可覆盖写入、挂载回读、checksum/解密/SQLite 校验、Apple“密码”owner-attested escrow、revision 一致的 fresh-clone restore-safe、脱敏账号审计 | implemented/gated：当前设备 SQLite、账号链路与私钥 escrow 已验证到各自边界；仍需另一设备实际取出/下载证明、周期保留、PostgreSQL 和完整接管演练 |
