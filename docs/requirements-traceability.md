...
| 超期监控、完成任务定期清理、自动化跟进清单每工作日评估 | dida-monitors | 契约测试；三类 monitor 均有现网运行时间且当前健康；completed-cleanup 超时保留 Claude/Codex 主执行与兜底阶段的脱敏结果 | complete：历史总超时已可在下一次失败时直接定位 provider 阶段，不读取或写入任务正文 |
