...
| 超期监控、完成任务定期清理、自动化跟进清单每工作日评估 | dida-monitors | 契约测试；三类 monitor 均有现网运行时间且当前健康；completed-cleanup 与工作日跟进超时均保留 Claude/Codex 主执行与兜底阶段的脱敏结果；跟进检查前两次失败静默、连续三次通知、仅已通知故障发送恢复 | complete：历史总超时已可在下一次失败时直接定位 provider 阶段，不读取或写入任务正文 |
