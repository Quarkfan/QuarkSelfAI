...
控制台采用 **Quiet Intelligence** 视觉方向：深石墨背景提供安静工作空间，荧光黄绿只表示品牌与需要决定的注意力，
雾化材质限于导航和顶栏，数据内容使用安静实体表面。首页先说明“今天需要关注什么”，再呈现开放事项、执行、批准和
异常；运行监控与技术栈位于第二层。导航使用统一的线性 SVG 图标，页面独特构图由 `web/console-redesign.css` 承担，
全局可访问性仍由最后加载的 `web/interface-baseline.css` 兜底。
