...
Initialization creates or recovers local encrypted identity state but does not connect, poll enrollment, discover executors, load capabilities, invoke an executor or enable effects. Enrollment begin/poll and one no-effect session cycle remain separate explicit methods. ADR 0134 adds explicit macOS Keychain provisioning; installer directory creation, background service lifecycle and non-macOS key providers remain unimplemented.
