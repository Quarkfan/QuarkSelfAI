...
## 2026-09-06 工作集成首批迁移预检

- 在未取得内容复制授权时继续做无副作用预检：从固定主线 revision 对 21 项私有候选逐项回读并核对内容 SHA-256，批次
  digest 为 `6c2a8904b82c109a5c2d8f999ee2ddad315415605b21c0000041db327b95bff5`。
- 预检推翻“21 项都可原样复制”的宽泛假设：14 项可在批准后原样复制，6 项回归证据必须重建为脱敏 replay，1 项因
  两处凭证形态测试字面量被排除待人工改造；工作保密提示只保留计数，不输出正文。
- 私有 revision `c44c14269c512279889ad78d90d2a9329b60838a` 已推送，严格审计确认批次逐项对应来源账本、秘密模式 0、owner 批准 0、
  内容复制 0、运行激活 0。本轮不修改源仓库运行文件、不重启、不触碰消费者或外部写入。

## 2026-09-06 工作集成首批内容迁移

- owner 精确批准批次 `6c2a8904b82c109a5c2d8f999ee2ddad315415605b21c0000041db327b95bff5` 后，先把执行器加固为全量
  校验通过才开始写入；冻结提案保持不变，批准和落盘事实由独立结果账本表达。
- 私有 revision `971685b22476e6b1e263b20f441b5ea72519dcf2` 已推送：14 项按固定 source revision 原样复制、6 项重建脱敏 replay、
  1 项含凭证形态 fixture 的测试排除。结果摘要为 `54e4ea05a735680361ca0cddb005890ef5fcad003df0e3eb2ae711addb3cb028`。
- 严格审计对账 20 个目标，秘密模式 0、符号链接 0、activation 0；迁移命令二次执行保持幂等。一个自包含 replay 实际通过，
