# 异机恢复资源申请单

下面资源是从“代码可 clone”推进到“任意受信终端可恢复”的必要输入。请不要在聊天中发送私钥、token、appSecret
或数据库密码；只需确认选择，并把秘密放入约定的密码管理器或目标主机 secret store。

## 需要 owner 选择的两项资源

### 1. 异机备份目标

推荐：一个仅 QuarkSelfAI 使用、启用版本化的 S3 兼容私有 bucket。

需要提供的非秘密信息：

- endpoint、region、bucket、可选 prefix；
- 保留/对象锁能力；
- 是否允许从新终端读取。

访问密钥只存密码管理器或服务 secret。若暂时没有对象存储，也可提供 NAS/加密云盘目录，但同一台电脑的本地目录
不算灾难恢复资源。

### 2. 备份加密身份

推荐：提供一个 `age` X25519 **公钥 recipient**。公钥可以写入非秘密配置；对应私钥保存在 1Password/系统钥匙串
和至少一处离线恢复位置，不进入仓库和备份 bucket。

如果不用 age，请指定具有同等“客户端加密、私钥与存储分离、可脚本化解密”能力的方案。

## 我将自行完成的后续工作

资源确认后，助手会：

1. 实现 SQLite online backup、PG custom dump、筛选文件归档和 SHA-256 manifest。
2. 实现客户端加密、上传、回读校验、14 日备份 + 8 周备份保留策略。
3. 建立 `restore-safe` 恢复命令，默认关闭 consumer 和外部 write effects。
4. 把账号登录、密钥注入、路径重写和单消费者切换写成唯一 runbook。
5. 在一个干净目录先做同机隔离演练；再约定一台新终端做真实恢复演练。

## 不需要现在提供

- Codex/Claude 的完整历史目录；
- BlackLake 业务数据库或内部仓库备份；
- 飞书、滴答或模型 token 的明文；
- 新服务器、域名或公网入口。首个恢复目标仍是本地终端。

## 建议默认值

- RPO：24 小时；RTO：60 分钟。
- 备份：每日一次，关键迁移前额外一次。
- 恢复包：仅最小权威状态；日志和会话历史不进入。
- 首个演练平台：另一 macOS 用户目录或独立 macOS 设备；Linux/容器作为下一阶段兼容验证。
