...
当前已创建并发布独立私有仓库 `QuarkSelfAI-Work` 的骨架；GitHub 页面已确认 visibility 为 `Private`，远端 `main` 在
revision `c44c14269c512279889ad78d90d2a9329b60838a` 保存 99 个来源资产的逐项内容摘要、分类、计划动作和审阅依据，但不复制命中
正文。候选结论为私有复制 21、主线泛化 64、脱敏回放 9、历史退休 5；owner 批准数、复制数和允许激活数均为 0。骨架
审计确认未启用消费者或外部写入，且账本可从固定主线 revision 完整重建。它现在是跨终端可取回的工作集成骨架真源，
但在内容迁移完成前仍不代表工作能力已经隔离或可以恢复运行。

21 项私有候选已经形成 digest 为 `6c2a8904b82c109a5c2d8f999ee2ddad315415605b21c0000041db327b95bff5` 的首批预检：
14 项可在批准后原样复制，6 项必须重建为脱敏 contract replay，1 项因凭证形态字面量被排除待人工改造。预检只保存路径、
摘要与风险计数，不复制正文；三类的 owner 批准、复制和激活数仍均为 0。
