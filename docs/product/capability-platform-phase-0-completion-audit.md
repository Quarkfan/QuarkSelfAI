...
| 现有能力无遗漏迁移 | `config/capability-platform-migration.json` | 原 99 项保持覆盖；Phase 1A/2A 新增模块后当前 module catalog 103/103 exactly-once |
...
`manage`、目标 screen 和唯一 `data-coverage` POC anchor。`scripts/audit-capability-platform-design.ts` 同时验证当前 103 个模块
