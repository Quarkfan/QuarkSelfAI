...
| 现有能力无遗漏迁移 | `config/capability-platform-migration.json`、`config/capability-artifact-candidates.json` | 原 99 项保持覆盖；Phase 1–4B 新增模块后当前 module catalog 109/109 exactly-once；55 个非核心、非迁移 Offer 归并为 24 个未发布 candidate |
...
`manage`、目标 screen 和唯一 `data-coverage` POC anchor。`scripts/audit-capability-platform-design.ts` 同时验证当前 109 个模块
