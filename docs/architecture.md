...
真实只读影子试运行仍由独立授权单控制，owner 可见交付不包含在当前候选中，详见
[ADR 0170](adr/0170-inactive-pre-meeting-briefing-planner.md)。
同一 seam 还提供纯计数退出门禁：只接受工作日、合格会议、草稿、人工复核和四类安全违规计数，按授权单的 8 个会议/10 个工作日、
80% 覆盖、70% 有效和 30% 低价值阈值产生 `continue/pass/fail`。它不读取简报内容、不执行来源读取，也不能激活 workflow 或 effect。
授权单 revision 3 另以 SHA-256 指纹绑定完整请求和既有实现基线，并把实现限制在同一个 inactive policy 模块；审计会拒绝范围、批准短语、
通知策略中的 revision、runtime、ownership 或禁止变更集合漂移。该指纹门禁不记录批准，也不允许提前接入真实来源。
