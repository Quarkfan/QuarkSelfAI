...
`dsh` CLI，后续 fallback adapter 必须按 bundled-runtime contract 接入，不能把 host binary 当成前置。当前共 126 个模块，
...
可接受通用 tenant identity；这不放宽云端 inactive provider 的 `test.*` 限制，也未选择生产 OS master-key provider 或分发入口。
