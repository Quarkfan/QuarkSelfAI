...
test('intake cheaply selects owner DM, focus people, owner cards, membership and owner reactions', async () => {
  ...
  try {
    ...
    await ctx.quarkIntake.handle({ ...base, kind: 'message.received', eventKey: 'im.message.receive_v1', deduplicationKey: 'dm', source: { ...base.source, actorId: 'owner' }, payload: { chatType: 'p2p', content: '自然语言目标' } })
    await ctx.quarkIntake.handle({ ...base, kind: 'message.received', eventKey: 'im.message.receive_v1', deduplicationKey: 'focus', source: { ...base.source, actorId: 'focus' }, payload: { chatType: 'group', content: '更新' } })
    await ctx.quarkIntake.handle({ ...base, kind: 'message.received', eventKey: 'im.message.receive_v1', deduplicationKey: 'other-dm', source: { ...base.source, actorId: 'colleague' }, payload: { chatType: 'p2p', content: '请审批' } })
    await ctx.quarkIntake.handle({ ...base, kind: 'message.received', eventKey: 'im.message.receive_v1', deduplicationKey: 'noise', source: { ...base.source, actorId: 'noise' }, payload: { chatType: 'group', content: '闲聊' } })
    await ctx.quarkIntake.handle({ ...base, kind: 'message.received', eventKey: 'quark.focus.discovered.v1', deduplicationKey: 'discovered', source: { ...base.source, actorId: 'noise', containerId: 'oc_dynamic' }, payload: { chatType: 'group', content: '来自动态关注会话' } })
