...
test('intake cheaply selects owner DM, focus people, owner cards, membership and owner reactions', async () => {
  ...
  try {
    ...
    await ctx.quarkIntake.handle({ ...base, kind: 'lark.event', eventKey: 'im.chat.member.user.added_v1', deduplicationKey: 'member', payload: { event: { operator_id: { open_id: 'delegate' }, users: [{ user_id: { open_id: 'owner' } }] } } })
    await ctx.quarkIntake.handle({ ...base, kind: 'lark.event', eventKey: 'im.message.reaction.created_v1', deduplicationKey: 'reaction', payload: { event: { user_id: { open_id: 'owner' } } } })
