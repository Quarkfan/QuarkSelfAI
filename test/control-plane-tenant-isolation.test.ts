...
test('queues only idempotent no-effect test dispatches and accepts privacy-bounded results', () => {
  ...
  const result = { taskId: 'task.001', deviceId: first.deviceId, planId: first.plan.planId, outcome: 'succeeded' as const, summaryCode: 'completed', artifactDigests: [sha], completedAt: later }
  assert.throws(() => store.complete(context, result), /acknowledged task lease/)
  ...
  assert.equal(store.complete(context, result).summaryCode, 'completed')
  assert.equal(store.complete(context, result), store.complete(context, result))
  assert.equal(store.getDispatch(context, 'task.001')?.state, 'completed')
  assert.throws(() => store.complete(context, { ...result, planId: 'plan.other' }), /outside the leased scope/)
  assert.throws(() => store.complete(context, { ...result, outcome: 'failed', summaryCode: 'failed' }), /immutable task result/)
  assert.throws(() => store.complete(context, { ...result, summaryCode: '/Users/demo/output', artifactDigests: [] }), /privacy bounded/)
