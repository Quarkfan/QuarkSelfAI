...
test('runs the built cloud server entry until a graceful signal', async t => {
  ...
  const configRoot = join(root, 'config'); const runtimeRoot = join(root, 'runtime'); await mkdir(configRoot, { mode: 0o700 }); await mkdir(runtimeRoot, { mode: 0o700 })
  const databasePath = join(root, 'control.sqlite3'); const socketPath = join(runtimeRoot, 'device.sock'); const instanceLeasePath = join(runtimeRoot, 'instance'); const keyPath = join(configRoot, 'key.pem'); const certPath = join(configRoot, 'cert.pem'); const configPath = join(configRoot, 'server.json')
  try {
    ...
    try { try { await waitForReady(child) } catch (error) { const result = await completion; if (result.code === 1 && result.stdout === '' && result.stderr === 'Cloud server failed: startup-or-runtime-failure\n') { t.skip('sandbox does not permit listeners'); return }; throw error }; assert.equal((await lstat(socketPath)).isSocket(), true); const second = await run(process.execPath, ['dist/control-plane/cloud-server-entry.js', 'run', configPath], { cwd: new URL('..', import.meta.url).pathname, env: { ...process.env, QUARK_CLOUD_SERVER_ENABLE: '1' } }).catch(error => error as { stdout: string; stderr: string; code: number }); assert.deepEqual({ stdout: second.stdout, stderr: second.stderr, code: second.code }, { stdout: '', stderr: 'Cloud server failed: startup-or-runtime-failure\n', code: 1 }); assert.equal((await lstat(socketPath)).isSocket(), true); child.kill('SIGTERM'); const result = await completion; assert.deepEqual(result, { code: 0, stdout: '{"schemaVersion":1,"state":"ready","transport":"tls+ssh","externalEffectsEnabled":false}\n', stderr: '' }); await assert.rejects(lstat(socketPath), error => (error as NodeJS.ErrnoException).code === 'ENOENT') }
    finally { if (child.exitCode === null) child.kill('SIGTERM') }
    await assert.rejects(lstat(instanceLeasePath), error => (error as NodeJS.ErrnoException).code === 'ENOENT')
    const crashed = spawn(process.execPath, ['dist/control-plane/cloud-server-entry.js', 'run', configPath], { cwd: new URL('..', import.meta.url).pathname, env: { ...process.env, QUARK_CLOUD_SERVER_ENABLE: '1' }, stdio: ['ignore', 'pipe', 'pipe'] }); const crashedCompletion = collect(crashed)
    await waitForReady(crashed); assert.equal((await lstat(socketPath)).isSocket(), true); assert.equal((await lstat(instanceLeasePath)).isDirectory(), true); crashed.kill('SIGKILL'); assert.equal((await crashedCompletion).code, null); assert.equal((await lstat(socketPath)).isSocket(), true); assert.equal((await lstat(instanceLeasePath)).isDirectory(), true)
    const recovered = spawn(process.execPath, ['dist/control-plane/cloud-server-entry.js', 'run', configPath], { cwd: new URL('..', import.meta.url).pathname, env: { ...process.env, QUARK_CLOUD_SERVER_ENABLE: '1' }, stdio: ['ignore', 'pipe', 'pipe'] }); const recoveredCompletion = collect(recovered)
    try { await waitForReady(recovered); recovered.kill('SIGTERM'); assert.deepEqual(await recoveredCompletion, { code: 0, stdout: '{"schemaVersion":1,"state":"ready","transport":"tls+ssh","externalEffectsEnabled":false}\n', stderr: '' }) } finally { if (recovered.exitCode === null) recovered.kill('SIGTERM') }
    await assert.rejects(lstat(socketPath), error => (error as NodeJS.ErrnoException).code === 'ENOENT'); await assert.rejects(lstat(instanceLeasePath), error => (error as NodeJS.ErrnoException).code === 'ENOENT')
