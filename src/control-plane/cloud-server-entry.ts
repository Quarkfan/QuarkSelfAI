...
async function loadConfig(path: string): Promise<CloudServerEntryConfigV1 & { readonly root: string }> {
  ...
  try {
    ...
    if (Object.keys(value).sort().join(',') !== keys.sort().join(',') || value.schemaVersion !== 1 || !isRecord(value.runtime) || typeof value.instanceLeasePath !== 'string' || value.instanceLeasePath !== resolve(dirname(root), 'runtime/instance') || !isRecord(value.tlsCredentials) || !isRecord(value.planVerification) || Object.keys(value.tlsCredentials).sort().join(',') !== 'certPath,keyPath' || Object.keys(value.planVerification).sort().join(',') !== 'keyId,publicKey' || typeof value.tlsCredentials.keyPath !== 'string' || typeof value.tlsCredentials.certPath !== 'string' || typeof value.planVerification.keyId !== 'string' || typeof value.planVerification.publicKey !== 'string') throw new Error('cloud server entry config is invalid')
