    ...
    return [{
      ...
      options: options.slice(0, 20).map((option, index) => {
        ...
        return {
          ...
          value: JSON.stringify({ effectId: effect.id, eventType, value: item?.value }),
        }
      }),
    }]
  }
  const approvalId = text(effect.payload.approvalId, 'approvalId', 300)
  const formName = `owner_${effect.id.replace(/[^a-z0-9]/gi, '').slice(-12) || 'action'}`
  const inputOnly = mode === 'input'
  const form: JsonObject = {
    tag: 'form', name: formName, vertical_spacing: '12px',
    elements: [
      {
      ...
      { tag: 'button', name: `submit_${formName}`, text: { tag: 'plain_text', content: inputOnly ? '提交' : '提交补充说明' }, type: inputOnly ? 'primary_filled' : 'default', width: 'fill', form_action_type: 'submit' },
    ],
  }
  if (inputOnly) return [form]
  return [{
    tag: 'column_set', flex_mode: 'bisect', horizontal_spacing: '8px', columns: [
      { tag: 'column', width: 'weighted', weight: 1, elements: [{ tag: 'button', text: { tag: 'plain_text', content: text(effect.payload.confirmText, 'confirmText', 100) ?? '确认' }, type: 'primary_filled', width: 'fill', behaviors: [{ type: 'callback', value: { effectId: effect.id, approvalId, eventType, decision: 'approved' } }] }] },
      { tag: 'column', width: 'weighted', weight: 1, elements: [{ tag: 'button', text: { tag: 'plain_text', content: text(effect.payload.declineText, 'declineText', 100) ?? '暂不处理' }, type: 'default', width: 'fill', behaviors: [{ type: 'callback', value: { effectId: effect.id, approvalId, eventType, decision: 'declined' } }] }] },
    ],
  }, form]
}
