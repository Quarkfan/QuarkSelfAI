...
function taskEffect(state: FollowupOutreachState, now: string): WorkflowEffectInput { return { id: stable('followup-task-update', state.requestId, state.replyMessageId ?? '', String(state.sequence)), kind: TASK_PROJECTION_EFFECTS.recordFollowupReply, availableAt: now, payload: { taskId: state.taskId, requestId: state.requestId, contact: state.contact, replyMessageId: state.replyMessageId, replyContent: state.replyContent, replyReceivedAt: state.replyReceivedAt, ...(state.replyUrl ? { replyUrl: state.replyUrl } : {}), idempotencyKey: externalKey(state, `reply:${state.replyMessageId}`) } } }
