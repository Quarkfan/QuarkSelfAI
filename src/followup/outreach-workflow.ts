...
function interactionEffect(state: FollowupOutreachState, now: string): WorkflowEffectInput {
  ...
  if (state.candidates.length) return { id: stable('followup-contact-choice', state.requestId, String(state.sequence)), kind: ASSISTANT_EFFECTS.requestInteraction, availableAt: now, payload: { mode: 'choice', title: '选择跟进联系人', prompt: `“${state.query ?? state.personName ?? ''}”匹配到多个联系人`, eventType: 'followup.contact-selected', options: state.candidates.map(x => ({ label: `${x.name}${x.department ? ` · ${x.department}` : ''}`, value: x.openId })), idempotencyKey: externalKey(state, `choice:${state.sequence}`) } }
  return { id: stable('followup-contact-input', state.requestId, String(state.sequence)), kind: ASSISTANT_EFFECTS.requestInteraction, availableAt: now, payload: { mode: 'input', title: '补充跟进联系人', prompt: `没有找到联系人“${state.query ?? state.personName ?? ''}”，请填写更准确的姓名或企业邮箱。`, eventType: 'followup.contact-query', idempotencyKey: externalKey(state, `input:${state.sequence}`) } }
}
function sendEffect(state: FollowupOutreachState, now: string): WorkflowEffectInput { if (!state.contact) throw new Error('followup contact is missing'); return { id: stable('followup-send', state.requestId, String(state.sequence)), kind: LARK_EFFECTS.sendAsUser, availableAt: now, payload: { openId: state.contact.openId, content: `**我是常东旭的 AI 分身。** 受他授权，我正在协助跟进事项“${state.title}”。\n\n想向你确认：${state.question}\n\n背景：${state.context}\n\n你的回复会由我整理后反馈给常东旭，并同步到对应的跟进任务中。`, idempotencyKey: externalKey(state, 'send') } } }
function taskEffect(state: FollowupOutreachState, now: string): WorkflowEffectInput { return { id: stable('followup-task-update', state.requestId, state.replyMessageId ?? '', String(state.sequence)), kind: TASK_EFFECTS.recordFollowupReply, availableAt: now, payload: { taskId: state.taskId, requestId: state.requestId, contact: state.contact, replyMessageId: state.replyMessageId, replyContent: state.replyContent, replyReceivedAt: state.replyReceivedAt, ...(state.replyUrl ? { replyUrl: state.replyUrl } : {}), idempotencyKey: externalKey(state, `reply:${state.replyMessageId}`) } } }
...
export function followupOutreachWorkflow(config: FollowupOutreachConfig = {}): WorkflowDefinition {
  ...
  return { kind: 'followup.outreach', version: 1,
    ...
    reduce(raw, event): WorkflowDecision { const state = raw as FollowupOutreachState
      ...
      if (event.type === 'approval.approved' && state.phase === 'awaiting-approval') { if (event.payload.approvalId !== state.approvalId) throw new Error('approval correlation mismatch'); const next = { ...state, phase: 'sending' as const, sequence: state.sequence + 1 }; return { status: 'waiting', state: next, wakeAt: null, effects: [sendEffect(next, event.occurredAt)] } }
      ...
      if (event.type === 'effect.failed') { if (state.phase === 'resolving' && kind(event) === LARK_EFFECTS.resolveContact) return retry({ ...state, phase: 'ready' }, 'resolve-contact', event); if (state.phase === 'sending' && kind(event) === LARK_EFFECTS.sendAsUser) return retry(state, 'send', event); if (state.phase === 'updating-task' && kind(event) === TASK_EFFECTS.recordFollowupReply) return retry(state, 'task-update', event); if (state.phase === 'notifying' && kind(event) === ASSISTANT_EFFECTS.notifyOwner) return retry(state, 'notify', event) }
