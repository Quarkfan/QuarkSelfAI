...
async function matchingKnowledge(workspaceRoot, indexText, query, limit = 3) {
  const normalizedQuery = String(query || "").toLowerCase().replace(/\s+/g, " ");
  if (!normalizedQuery.trim()) return [];
  const indexDir = path.join(workspaceRoot, "ai/ai-devops-knowledge-base/indexes");
  const candidates = [];
...
export async function loadBlacklakeCapabilityContext(workspaceRoot, { query = "" } = {}) {
  const sections = [];
  let knowledgeIndex = "";
  ...
  const matches = await matchingKnowledge(workspaceRoot, knowledgeIndex, query);
  if (matches.length) {
    sections.push(`### 按当前消息与上下文命中的知识详情\n${matches.map(({ relativePath, content }) => `#### ai/ai-devops-knowledge-base/indexes/${relativePath}\n${compact(content, 8000)}`).join("\n\n")}`);
  }
