...
export class DidaTaskCreator {
  ...
  async doCreateFromMention(message, contextMessages, researchDecisionHistory) {
    ...
    const prompt = `用户已明确授权：把飞书重点消息（@常东旭、特别关注联系人在共同群的发言、他人发给常东旭的私聊、有效标记所属会话的新消息）自动转成滴答清单任务，并根据消息与常东旭的关系自动填写优先级、标签、明确的时间信息。你必须使用 dida365 MCP，不能只描述操作。
...
7. 标签必须有 2–5 个且不重复，优先复用已有稳定标签：
   ...
   - 剩余额度优先选择一个动作标签（待回复/待决策/待确认/待协调/待跟进）和一个场景标签（客户/生产/故障/黑湖/发布/项目）；特别关注触发可使用“特别关注”，私聊触发可使用“私聊”，标记会话触发可使用“标记会话”，但不得挤占更重要的动作/业务标签。
