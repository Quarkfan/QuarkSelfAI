...
try {
  await mkdir(join(root, 'program/dist/control-plane'), { recursive: true, mode: 0o700 }); await mkdir(join(root, 'program/dist/client-runtime'), { recursive: true, mode: 0o700 }); await mkdir(join(root, 'program/migrations/control-plane-sqlite'), { recursive: true, mode: 0o700 }); await mkdir(join(root, 'program/deploy/launchd'), { recursive: true, mode: 0o700 }); await mkdir(join(root, 'program/deploy/systemd'), { recursive: true, mode: 0o700 })
  ...
  await cp(join(sourceRoot, 'deploy/launchd/com.quarkfan.quark-server.plist.template'), join(root, 'program/deploy/launchd/com.quarkfan.quark-server.plist.template'), { dereference: true }); await cp(join(sourceRoot, 'deploy/systemd/quark-server.service.template'), join(root, 'program/deploy/systemd/quark-server.service.template'), { dereference: true })
