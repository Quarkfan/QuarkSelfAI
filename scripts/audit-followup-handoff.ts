...
import { auditResourceHash, followupAuditConfig } from '../src/migration/compat-handoff-audit-config.js'
...
const auditConfig = followupAuditConfig(config)
const handoff = prepareFollowupHandoff(JSON.parse(stateBytes), { ...auditConfig, retryBaseMs: 120_000, retryMaxMs: 21_600_000 }, stateInfo.mtime.toISOString())
process.stdout.write(`${JSON.stringify({ statePathHash: auditResourceHash(statePath), mode: 'read-only', ...handoff.counts, workflows: handoff.workflows.length, projectIdHash: auditResourceHash(auditConfig.projectId), digest: handoff.digest }, null, 2)}\n`)
