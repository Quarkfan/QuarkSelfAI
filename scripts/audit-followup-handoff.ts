...
const projectId = string(config.followupProjectId)
if (!projectId) throw new Error('compat config has no followupProjectId')
const handoff = prepareFollowupHandoff(JSON.parse(stateBytes), {
  projectId,
  taskProjection: {
    projectId,
    authorization: {
      id: 'owner-policy:dida-followup-projection:v1', grantedBy: 'owner', grantedAt: '2026-08-20T00:00:00+08:00',
      scope: 'dida.task-projection', revision: 1, source: 'owner-directive:delegate-followup-task-management', projectId,
    },
  },
  timeZone: string(config.followupTimeZone), scheduledHour: number(config.followupScheduledHour), pollIntervalMs: number(config.followupPollIntervalMs),
  retryBaseMs: 120_000, retryMaxMs: 21_600_000,
}, stateInfo.mtime.toISOString())
process.stdout.write(`${JSON.stringify({ statePath, mode: 'read-only', ...handoff.counts, workflows: handoff.workflows.length, projectIdHash: createHash('sha256').update(projectId).digest('hex').slice(0, 16), digest: handoff.digest }, null, 2)}\n`)
